...
var server = net.createServer(function (socket) {
    ...
    uid = uid + 1;
    var mongoc = new net.Socket();
    mongoc.connect(MONGODB_PORT, MONGODB_HOST, function () 
    {
        console.log(clientId + " -> Server:");
    });

    mongoc.on("data", function (data) {
        parseMessage(data, clientId);
        printToAddress(clientId, data);
        socket.write(data);
    });

    mongoconn.set(clientId + ":" + uid, mongoc);
